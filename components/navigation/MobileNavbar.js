  ...
  {isOpen ? (
    ...
    <RxHamburgerMenu className="text-2xl" />
      ...
      {isOpen && (
       <div
  ref={boxRef}
  className="fixed inset-0 z-50 bg-[var(--card-bg)] text-[var(--foreground)] flex flex-col items-center justify-start pt-20 px-6 font-sans transition-all"
>
  {/* Profile Section */}
  ...
  <div className="w-full mt-10">
    <p className="text-sm font-semibold mb-2 text-[var(--muted)]">Theme</p>
    <button
      onClick={toggleTheme}
      className="w-24 h-10 flex items-center border-2 border-[var(--border-color)] rounded-full bg-[var(--input-bg)] relative transition-all"
    >
      <span
  ...
  {/* Logout Button */}
  <button
    className="mt-12 flex items-center justify-center gap-2 px-5 py-2 text-[var(--danger)] bg-[var(--danger)]/10 hover:bg-[var(--danger)]/20 rounded-lg text-base font-medium transition-all"
